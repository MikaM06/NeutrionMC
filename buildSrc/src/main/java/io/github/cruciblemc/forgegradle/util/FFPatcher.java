package io.github.cruciblemc.forgegradle.util;
...
/**
 * Copy of {@code net.minecraftforge.gradle.extrastuff.FFPatcher} (ForgeGradle 1.2) with the
 * SYNTHETICS regex rewritten to avoid catastrophic backtracking.
 *
 * The original pattern used {@code (?<return>.+?) (?<method>.+?)\(} which nests three unbounded
 * quantifiers; on large 1.8.9 decompiled classes stuffed with synthetic methods (e.g.
 * EnumConnectionState) the java.util.regex engine goes exponential and the build hangs forever.
 * Return types and method names never contain spaces or parentheses in decompiled output, so the
 * lazy dot-matchers are replaced with the linear character classes {@code [^\s(]+}.
 */
public class FFPatcher {
    ...
    private static final Pattern SYNTHETICS = Pattern.compile("(?m)(\\s*// \\$FF: (synthetic|bridge) method(\\r\\n|\\n|\\r)){1,2}\\s*(?<modifiers>(?:(?:" + MODIFIERS + ") )*)(?<return>[^\\s(]+) (?<method>[^\\s(]+)\\((?<arguments>.*)\\)\\s*\\{(\\r\\n|\\n|\\r)\\s*return this\\.(?<method2>[^\\s(]+)\\((?<arguments2>.*)\\);(\\r\\n|\\n|\\r)\\s*\\}");
